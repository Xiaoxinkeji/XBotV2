...
本项目依赖特殊包 `xywechatpad-binary`，这是一个微信API的二进制扩展。该包不在公共PyPI仓库中，需要以下方式获取：

1. 从项目源代码的 `xywechatpad-binary` 目录安装
2. 从项目的 `wheels` 目录安装预编译的wheel文件
3. 如果没有上述资源，微信API相关功能将不可用

## 安装说明

如果没有二进制包，系统会优雅降级，禁用微信API功能但保持其他功能正常运行。

## 特殊依赖获取

请联系项目原作者获取 `xywechatpad-binary` 包的安装方法或二进制文件。
